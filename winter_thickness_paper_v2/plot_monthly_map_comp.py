...
for i in range(len(yearStrs)):
    lats_it, lons_it, icetype0=cF.getIceTypeLatLon(iceTypePath, '15', monStr, yearStrs[i], res=1)
    icetypes[i]=icetype0


# = is2['ice_type'].sel(time = strs[0]).values[0]
#icetypes[1] = is2['ice_type'].sel(time = strs[1]).values[0]

#icetypes[0] = is2['ice_type'].sel(time = strs[0]).values[0]
#icetypes[1] = is2['ice_type'].sel(time = strs[1]).values[0]

regions = [9, 10, 11, 12, 13, 15] #Inner Arctic
...
for i in range(len(data)):
    ...
    #im1 = ax.contourf(is2Lons , is2Lats, data[i], levels=np.arange(minval[i], maxval[i]+0.1, (maxval[i]-minval[i])/tickint*4), transform=ccrs.PlateCarree(), cmap=cm.cubehelix_r, vmin=minval[i], vmax=maxval[i], extend='both', shading='gouraud', edgecolors='None', zorder=2, rasterized=True)
    
    im1 = ax.pcolormesh(is2Lons , is2Lats, data[i] , cmap=cmaps[i], vmin=minval[i], vmax=maxval[i], transform=ccrs.PlateCarree(), zorder=2)
    
    if i<2:
        im11=ax.pcolormesh(is2Lons, is2Lats, iceconcs[i], vmin=0, vmax=2, cmap=plt.cm.gray_r, transform=ccrs.PlateCarree(), zorder=1)
    if i<2:
        im12 = ax.contour(lon2d_greater , lat2d_greater, regionMask_mask_g, colors='m', linewidths=1, levels=[0.5],transform=ccrs.PlateCarree(), zorder=3)
        im121 = ax.contour(lon2d_lesser , lat2d_lesser, regionMask_mask_l, colors='m', linewidths=1, levels=[0.5],transform=ccrs.PlateCarree(), zorder=3)
    
    if i<2:
        lon2dit_greater = ma.masked_greater(lons_it, -0.01)
        lon2dit_lesser = ma.masked_less(lons_it, 0)
        
        ice_type_mask_g = ma.MaskedArray(icetypes[i], mask=lon2dit_greater.mask)
        ice_type_mask_l = ma.MaskedArray(icetypes[i], mask=lon2dit_lesser.mask)
        lat2dit_greater = ma.MaskedArray(lats_it, mask=lon2dit_greater.mask)
        lat2dit_lesser = ma.MaskedArray(lats_it, mask=lon2dit_lesser.mask)
        #im13 = ax.contour(lons_it , lats_it, icetypes[i], color='k', linewidth=3, levels=[0.5],transform=ccrs.PlateCarree(), zorder=3)
        
        im13 = ax.contour(lon2dit_greater , lat2dit_greater, ice_type_mask_g, colors='k', linewidth=3, levels=[0.5],transform=ccrs.PlateCarree(), zorder=3)
        im131 = ax.contour(lon2dit_lesser , lat2dit_lesser, ice_type_mask_l, colors='k', linewidth=3, levels=[0.5],transform=ccrs.PlateCarree(), zorder=3)

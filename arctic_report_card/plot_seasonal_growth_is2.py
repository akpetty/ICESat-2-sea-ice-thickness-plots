...
def calcMeans(dataset, extraStr='_int'): 
    ...
    meanThickness = dataset[var+extraStr].mean(dim = ['x','y'], skipna = True)
...
extraStr='_int'
ylims = [0, 0.3] # [0.6, 2.4 ]
yticks = np.arange(0., 0.4, 0.1) # np.arange(0.5, 2.6, 0.5)
ylabel_str = 'Snow depth (m)'

is2means = calcMeans(is2, extraStr=extraStr)
...
plt.savefig(figPath+'seasonal_growth_is2'+label+var+extraStr+regionsText+'.pdf', dpi = 300)
